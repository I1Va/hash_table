...
## Бенчмаркинг версий
В бенчмаркинге меряется суммарное время выполнения функций `hash_table_32b_find_key` для набора входных данных.


Для измерения времени работы всех версий проекта исполните команду
```bash
    python3 run.py vbench
```
Количество измерений на версию можно поменять в `configs/benchmarks.yaml`. Для повышения точности можно установить количество холостых операций `find_key` перед началом основных измерений.

хэш-таблице в параметре `repeats`.

![image](/results/screenshots/runs.png)


После выполнения команды в папке `results` появится папка `versions_benchmark.out` с результатами бенчмаркинга с учетом погрешности ($x_{mean} ± 	\sigma_{x_{mean}}$). Выбросы были убраны по правилу $3‑σ$.
...
* `-g` - включает запись call graph для каждого сэмпла
* `--call-graph dwarf` - выбирает технологию `dwarf` сбора данных для `--call-graph`. При выборе `dwarf` `perf` дополнительно к исполняемой программе поддерживает стэковую память самостоятельно. Эта технология имеет ограничение на глубину стэка и требует больше ресурсов, __но__ позволяет анализировать программы, в которых в целях оптимизации не всегда создается стэковый фрэйм (см. `-fno-omit-frame-pointer`).
* `-T | --timestamp` - записывает временные метки (таймстемпы) для каждого сэмпла. Это позволяет в дальнейшем выводить точное время появления каждого события (например, с помощью perf report -D)
* `-F | --freq` - задаёт частоту сэмплирования — количество сэмплов в секунду

__Просмотр `perf hist`__
```
...
В учебных целях измерим время работы версии v1 несмотря на остутсвтие компиляторных оптимизаций.


__Результаты__
Версия | Ticks (±)  | Порядок | Ускорение относительно предыдущей версии
-------|------------|---------|-------
v1	   | 1014   ± 2	| 4       | -
...
__Perf hist__
![image](/results/screenshots/perfhist_v1.png)

__Flame graph__
![image](/results/screenshots/hotspot_v1.png)
...
__Perf hist__
![image](/results/screenshots/perfhist_v2.png)


При анализе ассемблерного кода после транлсяции было замечено, что компилятор ускорил хэш-функцию `cr32`.
...
### Версия `v3`
Не смотря на компиляторные оптимизации, примененные в `v2`, хэш-функция `cr32` по-прежнему остается самым узким местом программы. В `v3` воспользуемся `intrinsic` реализацией функции `cr32`.
...
__Perf hist__
![image](/results/screenshots/v3perfhist.png)


Анализ `perf hist` показал, что примененная оптимизация дала __ощутимый эффект__: `cr32` стала занимать всего `2.34% overhead`. Теперь стандратная функция `strncmp_avx2` является узким местом программы с `56.89% overhead`.
...
__Perf hist__
![image](/results/screenshots/v4streq.png)
...
```
По дизасемблеру видно, что произошел `inlining` функции `streq`.
![image](/results/screenshots/inlinestreq.png)
...
Оптимизация дала незначительный прирост к скорости. Данный прирост достаточно незначителен, чтобы закончить на нем цепочку оптимизаций.
...
Итого, итоговая версия `v5` имеет ускорение в `3.86` раза относительно версии c компиляторными оптимизациями `v2`.

# Вывод:
После процесса последовательных оптимизаций работа с хеш-таблицей была ускорена в 7.5 раз. Было продемонстрировано, что при должном понимании ограничений на входные данные появляется возможность проводить более агрессивные оптимизации, которые не может себе позволить компилятор. Также был показан ощутимый эффект применения ассемблерных оптимизаций.
